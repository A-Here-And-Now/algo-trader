...
type IPriceActionStore interface {
	...
	GetRenkoCandleHistory(symbol string) *RenkoCandleHistory

	BuildRenkoCandleHistory(brickSize float64)
	IsRenkoCandleHistoryBuilt() bool
}
...
type PriceActionStore struct {
	...
	renkoCandleHistory      map[string]*RenkoCandleHistory
	isRenkoCandleHistoryBuilt bool
...
func NewStore(strategy enum.Strategy) *PriceActionStore {
	store := PriceActionStore{
		...
		renkoCandleHistory:      make(map[string]*RenkoCandleHistory),
...
func (s *PriceActionStore) UpdateStrategy(strategy enum.Strategy) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.strategy = strategy
	s.renkoCandleHistory = nil
	s.isRenkoCandleHistoryBuilt = false
}

func (s *PriceActionStore) GetRenkoCandleHistory(symbol string) *RenkoCandleHistory {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.renkoCandleHistory[symbol]
}

func (s *PriceActionStore) IsRenkoCandleHistoryBuilt() bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.isRenkoCandleHistoryBuilt
}
...
func (s *PriceActionStore) IngestPrice(symbol string, ticker models.Ticker) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.priceHistory[symbol] = append(s.priceHistory[symbol], ticker)
	buf := s.priceHistory[symbol]
	const maxTicks = 1200 // ~100 minutes if ~1/5s
	if len(buf) > maxTicks {
		buf = buf[len(buf)-maxTicks:]
	}
	s.priceHistory[symbol] = buf

	if (s.strategy == enum.RenkoCandlesticks && s.isRenkoCandleHistoryBuilt){
		brickSize := s.renkoCandleHistory[symbol].BrickSize
		lastClose := s.renkoCandleHistory[symbol].LastCandlePrice
		price, err := strconv.ParseFloat(ticker.Price, 64)
		if err != nil {
			log.Printf("RENKO ERROR: Failed to parse %s price of %s: %v", ticker.ProductID, ticker.Price, err)
		}
		if (math.Abs(price-lastClose) >= brickSize){
			newRenkoCandles, newLastClose := GetNewRenkoCandles(price, lastClose, brickSize)
			s.renkoCandleHistory[symbol].RenkoCandles = append(s.renkoCandleHistory[symbol].RenkoCandles, newRenkoCandles...)
			s.renkoCandleHistory[symbol].LastCandlePrice = newLastClose
		}
	}
}
